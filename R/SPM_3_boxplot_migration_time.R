...
Ubqdf<-df%>%
  ...
  mutate(`migration_time(min)`=as.numeric(`migration_time(min)`))
G<-c("50_stirred","75_stirred","75_unstirred")
UbqMT<-ggplot(data = Ubqdf,aes(x=capillary,y=`migration_time(min)`))+
...
Myodf<-df%>%
  ...
  mutate(`migration_time(min)`=as.numeric(`migration_time(min)`))
MyoMT<-ggplot(data = Myodf,aes(x=capillary,y=`migration_time(min)`))+
  geom_boxplot(aes(fill = capillary),
               color=spmcolor[2],
               width=boxsize+0.4,
               outlier.shape = 8,#隐藏outlier
...
CAdf<-df%>%
  ...
  mutate(`migration_time(min)`=as.numeric(`migration_time(min)`))
CAMT<-ggplot(data = CAdf,aes(x=capillary,y=`migration_time(min)`))+
  geom_boxplot(aes(fill = capillary),
               color=spmcolor[3],
               width=boxsize+0.4,
               outlier.shape = 8,#隐藏outlier
...
MTall<-wrap_plots(UbqMT, MyoMT, CAMT, nrow = 1)
ggsave(plot =MTall,"pic/SPM/Migration_all.pdf",width = 210,height=70,units = "mm",dpi = 600)
