...
Ubqdf<-df%>%
  ...
  mutate(FWHM=as.numeric(FWHM))
G<-c("50_stirred","75_stirred","75_unstirred")
UbqFWHM<-ggplot(data = Ubqdf,aes(x=capillary,y=FWHM))+
...
UbqFWHM
#ggsave(plot =UbqFWHM,"pic/SPM/Migration_Ubq.tiff",width = 70,height=70,units = "mm",dpi = 600)
...
Myodf<-df%>%
  ...
  mutate(FWHM=as.numeric(FWHM))
MyoFWHM<-ggplot(data = Myodf,aes(x=capillary,y=FWHM))+
  geom_boxplot(aes(fill = capillary),
               color=spmcolor[2],
               width=boxsize+0.4,
               outlier.shape = 8,#隐藏outlier
...
CAdf<-df%>%
  ...
  mutate(FWHM=as.numeric(FWHM))
CAFWHM<-ggplot(data = CAdf,aes(x=capillary,y=FWHM))+
  geom_boxplot(aes(fill = capillary),
               color=spmcolor[3],
               width=boxsize+0.4,
               outlier.shape = 8,#隐藏outlier
  ...
  stat_compare_means(comparisons = list(c(G[1],G[2]),c(G[1],G[3]),c(G[2],G[3])),
                     ...
                     tip.length = 0.003,      # 线条末端小短线的长度
                     step.increase = 0.05) +     # 每组标注的高度递增值
  labs(title="Carbonic anhydrase",x="",y="FWHM")+
  xlim(G)+
  theme(plot.title = element_blank(),
        axis.title =  element_blank(),
...
FWHMall<-wrap_plots(UbqFWHM, MyoFWHM, CAFWHM, nrow = 1)
ggsave(plot =FWHMall,"pic/SPM/FWHM_all.pdf",width = 210,height=70,units = "mm",dpi = 600)
